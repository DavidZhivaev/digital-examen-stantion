...
from output_module.output_generator_wrapper import (
    OutputGenerator,
    CPP_AVAILABLE,
    PackageResult,
)
...
def _decode_veroytn_digits(veroytn: Any, expected_len: int) -> str:
    if veroytn is None:
        return ""
    field: Optional[List] = None
    if isinstance(veroytn, list):
        ...
        for key in ("digits", "number", "page", "list", "sheet"):
            if isinstance(veroytn.get(key), list):
                field = veroytn[key]
                break
        if field is None and len(veroytn) == expected_len:
            field = [veroytn.get(str(i), {}) for i in range(expected_len)]
    if not field or len(field) != expected_len:
        return ""
    digits = "0123456789"
    chars: List[str] = []
    for pos in field:
...
def _extract_recognized_numbers(blanks: Sequence["ScannedBlank"]) -> Dict[str, str]:
    ...
    for blank in blanks:
        ...
        if type_code not in CHAIN_TYPES:
            continue
        barcode = resolve_blank_id(blank)
        if barcode is None:
            continue
        barcode_str = f"{barcode:013d}"
        veroytn = extract_veroytn(blank)
        if veroytn is None:
            continue
        if type_code == "blan2":
            decoded = _decode_veroytn_digits(veroytn, 3)
        else:
            decoded = _decode_veroytn_digits(veroytn, 13)
        if decoded:
            recognized[barcode_str] = decoded
    return recognized
...
def export_work_zip(
    ...
    if not CPP_AVAILABLE:
        return {
            "ok": False,
            "message": "C++ output module not available. Build output_generator_cpp.pyd first.",
        }
    ...
    links = auditorium_result.get("links", {})
    recognized = _extract_recognized_numbers(blanks)

    chain_blanks = [
        b for b in blanks
        if b.qr_info and b.qr_info.type_code in CHAIN_TYPES
        and not getattr(b, "is_corrupted", False)
    ]

    if not chain_blanks:
        return {"ok": False, "message": "Нет бланков для экспорта"}

    title_barcode: Optional[int] = None
    for blank in chain_blanks:
        if blank.qr_info and blank.qr_info.type_code == "titul":
            title_barcode = resolve_blank_id(blank)
            break
    if title_barcode is None and chain_blanks:
        title_barcode = resolve_blank_id(chain_blanks[0])

    chain_valid = auditorium_result.get("auditorium_status") in {
        AUDITORIUM_STATUS_CLEAN,
        AUDITORIUM_STATUS_AUTO_FIXED,
        AUDITORIUM_STATUS_FINALIZED,
    }

    generator = OutputGenerator(str(output_path.parent))
    ...
    if not result.ok:
        return {
            "ok": False,
            "message": f"Export failed with status {result.status.name}",
    ...
    return {
        ...
        "saved": result.sheet_count,
        "path": result.zip_path,
        "pdf_filename": result.pdf_filename,
        "title_barcode": result.title_barcode,
