...
[[nodiscard]] constexpr auto buildResultsJson(const WorkChain& chain) noexcept -> JsonBuilder<BufferSize> {
    ...
    for (const auto& sheet : chain.sheets) {
        if (sheet.type == SheetType::Titul) [[unlikely]] {
            continue;
        }

        if (!sheet.isValid()) [[unlikely]] {
            continue;
        }

        auto sheetKey{formatBarcode(sheet.barcode)};
        builder.keyBeginObject(sheetKey);

        if (sheet.hasLink()) [[likely]] {
            auto linkedStr{formatBarcode(sheet.linkedBarcode)};
            builder.keyValue("linked_to", linkedStr);
        } else {
            builder.key("linked_to");
            builder.valueString("");
        }

        if (sheet.hasRecognized()) [[likely]] {
            builder.keyValue("recognized", sheet.recognizedDigits);
        } else {
            builder.key("recognized");
            builder.valueString("");
        }
