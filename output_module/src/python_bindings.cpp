...
public:
    ...
    auto createPackage(
        ...
        const std::vector<std::tuple<std::string, std::int64_t, int, std::int64_t, std::string>>& sheets,
        bool chainValid
    ) -> py::dict {
        WorkChain chain{};
        ...
        SizeType order{0};
        for (const auto& [path, barcode, typeInt, linkedTo, recognized] : sheets) {
            SheetData sheet{};
            ...
            sheet.linkedBarcode = linkedTo;
            sheet.recognizedDigits = StaticString<16>{recognized.c_str()};
            sheet.valid = true;
