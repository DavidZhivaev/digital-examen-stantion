...
struct alignas(CACHE_LINE_SIZE) SheetData {
    ...
    BarcodeType linkedBarcode{0};
    StaticString<16> recognizedDigits{};
    bool valid{false};
    ...
    constexpr SheetData(
        ...
        , type{t}
        , orderInChain{order}
        , linkedBarcode{linked}
        , recognizedDigits{recognized}
        , valid{true} {}
    ...
    [[nodiscard]] constexpr auto hasLink() const noexcept -> bool {
        return linkedBarcode >= BARCODE_MIN && linkedBarcode <= BARCODE_MAX;
    }

    [[nodiscard]] constexpr auto hasRecognized() const noexcept -> bool {
        return !recognizedDigits.empty();
    }
};
