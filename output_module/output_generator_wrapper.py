...
class OutputGenerator:
    ...
    def create_package(
        ...
        recognized: Optional[Mapping[str, str]] = None,
        ...
        sheets: List[Tuple[str, int, int, int, str]] = []
        for i, blank in enumerate(blanks):
            path = _save_temp_image(blank, temp_dir, i)
            if path is None:
                continue
            barcode = _resolve_blank_id(blank) or 0
            barcode_str = f"{barcode:013d}" if barcode else ""
            type_code = ""
            if blank.qr_info and blank.qr_info.type_code:
                type_code = blank.qr_info.type_code
            type_int = _type_code_to_enum(type_code)
            linked_to = int(links.get(barcode_str, "0") or "0")
            recognized_digits = recognized.get(barcode_str, "")
            sheets.append((path, barcode, type_int, linked_to, recognized_digits))
